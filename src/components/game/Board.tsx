...
function Board() {
  ...
  const prevWallCount = useRef(gameState.walls.length);

  useEffect(() => {
    if (gameState.walls.length > prevWallCount.current) {
      setIsHovered(null);
    }
    prevWallCount.current = gameState.walls.length;
  }, [gameState.walls]);

  return (
    <div className="w-full max-w-140 aspect-square mx-auto p-2">
      <section
        dir="ltr"
        className="grid w-full h-full p-2"
        style={{
          gridTemplateColumns: "repeat(8, 3fr 1fr) 3fr",
          gridTemplateRows: "repeat(8, 3fr 1fr) 3fr",
        }}
      >
